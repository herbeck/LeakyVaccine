...
This is just following from Mia Moore and Dobromir Dimitrov, "Overwhelming of the vaccine efficacy in HVTN 702 by repeated exposure."
...
dYdt[2] = -r*(1-f)*Sp + r*f*Up - Sp*(1-(1-rho)^theta) #rho*theta*Sp
dYdt[3] = r*(1-f)*Sv - r*f*Uv
dYdt[4] = -r*(1-f)*Sv + r*f*Uv - Sv*   #(rho*theta)*(1-(epsilon*Sv))
...
```{r parameter values}
rho <-      0.03  #par[1]    
epsilon <-  0.54  #par[2] #Pass as vector for years 1 and 2 and 0.54 and 0.28
f <-        0.02  #par[3]      
r <-        0.03  #par[4]      
theta <-    48    #par[5]  

init  <- c(0.25, 0.25, 0.25)
t     <- seq(0, 3, 0.1)
