...
Note that this code chunk includes the model initiation piece, in which among other things we initialize the overall population size and the size of any subpopulations defined by exposure risk. 
...
init <- init.dcm(Sp = 10000, Ip = 0,  #placebo, homogeneous risk population
                 Sv = 10000, Iv = 0,  #vaccine, homogeneous risk population
                 ...
                 Svm = 6000, Ivm = 0,    #vaccine, medium risk
                 Svl = 3000, Ivl = 0,    #vaccine, low risk
...
#### Start with homogeneous risk
Let's start by showing two populations with homogeneous risk but different forces of infection (incidence in the placebo arm). Let's set the first population incidence at 0.3 infections/100 py.

Let's plot the cumulative infections in each population.

Run the model with low incidence parameters (0.03 inf/100py)
```{r parameters low incidence homogeneous}
...
```{r}
lambda <- lambda*2   #transmission rate (per contact)
...
init <- init.dcm(Sp = 10000, Ip = 0,  #placebo, homogeneous risk population
                 ...
                 Svl = 2000, Ivl = 0,    #vaccine, low risk
                 SIp.flow = 0, SIv.flow = 0, 
                 SIph.flow = 0, SIpm.flow = 0, SIpl.flow = 0,
                 SIvh.flow = 0, SIvm.flow = 0, SIvl.flow = 0)
...
plot(mod, y = c("Ip", "total.Iph.Ipm.Ipl"),
     ...
     xlab = "days",
     col = c("blue", "red"))
legend("bottomright", legend = c("homogeneous risk, incidence = 0.3", "homogeneous risk, incidence = 3.0"), col = c("blue", "red"), lwd = 2, cex = 0.9, bty = "n")
```



Let's show the cumulative infections in a (placebo) population with and without exposure heterogeneity, over the course of a standard 3 year trial. The population size is 10000. 
...
Now let's plot hazard / instantaneous incidence in these two populations.  
...
#placebo.incidence <- c(0.39, 0.32, 0.28)    # RV144 incidence per 100 person years; adjust accordingly
placebo.incidence <- c(3.0, 3.0, 3.0)  # HVTN 702 incidence
...
f <- function(x) {
  ...
  init <- init.dcm(Sp = 5000, Ip = 0,
                 Sv = 5000, Iv = 0,
                 Sph = 1000, Iph = 0,    #placebo, high risk
                 Spm = 3000, Ipm = 0,   #placebo, medium risk
                 Spl = 6000, Ipl = 0,    #placebo, low risk
                 Svh = 1000, Ivh = 0,    #vaccine
                 Svm = 3000, Ivm = 0,    #vaccine
                 Svl = 6000, Ivl = 0,    #vaccine
                 SIp.flow = 0, SIv.flow = 0, 
                 SIph.flow = 0, SIpm.flow = 0, SIpl.flow = 0,
                 SIvh.flow = 0, SIvm.flow = 0, SIvl.flow = 0)
  control <- control.dcm(nsteps = 365*3, new.mod = si_ode)
...
param <- param.dcm(beta = 0.004, 
                     c = 90/365,
                     prev = 0.10,    
                     #lambda = median(fit$param[,1]), #4.323494e-06
                     lambda = 5.2e-06, 
                     epsilon = 0.3,
...
f <- function(x) {
  ...
  init <- init.dcm(Sp = 5000, Ip = 0,
                 Sv = 5000, Iv = 0,
                 Sph = 2000, Iph = 0,    #placebo, high risk
                 Spm = 5000, Ipm = 0,    #placebo, medium risk
                 Spl = 3000, Ipl = 0,    #placebo, low risk
                 Svh = 2000, Ivh = 0,    #vaccine
                 Svm = 5000, Ivm = 0,    #vaccine
                 Svl = 3000, Ivl = 0,    #vaccine
                 SIp.flow = 0, SIv.flow = 0, 
                 SIph.flow = 0, SIpm.flow = 0, SIpl.flow = 0,
                 SIvh.flow = 0, SIvm.flow = 0, SIvl.flow = 0)
  control <- control.dcm(nsteps = 365*3, new.mod = si_ode)
...
plot(density(fit$param[, 1], from = 0.0000001,  to = 0.000005),
     ...
     #ylim = c(0, 10),
     col=2)
plot(density(fit$param[, 2], from = 5,  to = 15),
     main = "risk",
     xlab = "risk multiplier",
     xlim = c(5, 15),
     #ylim = c(0, 10),
...
How does this scenario fit to the target stats? (10000 total, 2000 in high risk group, 4k in both medium and low risk groups)

```{r}

#(10000 total, 2000 in high risk group, 4k in both medium and low risk groups)

f(12)
# 0.3383124 0.3353720 0.3324564
target.stats$placebo.incidence
# 0.39 0.32 0.28
```
...
Let's use our poorly calibrated RV144 model, with ~0.35% incidence, add our vaccine with epsilon ~30%, and then increase the `lambda` to 3.5% and see what VE is.
...
init <- init.dcm(Sp = 5000, Ip = 0,
                 ...
                 Sph = 1000, Iph = 0,    #placebo, high risk
                 Spm = 5000, Ipm = 0,   #placebo, medium risk
                 Spl = 5000, Ipl = 0,    #placebo, low risk
                 Svh = 1000, Ivh = 0,    #vaccine
                 Svm = 5000, Ivm = 0,    #vaccine
                 Svl = 5000, Ivl = 0,    #vaccine
                 SIp.flow = 0, SIv.flow = 0, 
                 SIph.flow = 0, SIpm.flow = 0, SIpl.flow = 0,
                 SIvh.flow = 0, SIvm.flow = 0, SIvl.flow = 0)
...
```{r}
par(mfrow = c(1, 1))
plot(mod, y=c("rate.Placebo.het", "rate.Vaccine.het"),
     alpha = 0.8,
     ylim = c(0, 0.5),
     main = "Hazard",
     xlab = "days",
...
plot(mod, y=c("rate.Placebo.het", "rate.Vaccine.het"),
     ...
     legend = FALSE,
     col = c("blue", "green"))
legend("bottomright", legend = c("placebo, heterogeneous risk", "vaccine, heterogeneous risk"), col = c("blue", "green"), lwd = 2, cex = 0.9, bty = "n")
```
...
plot(mod, y=c("VE2.inst"),
     ...
     ylab = "Vaccine efficacy",
     col = c("blue", "red"))
legend("topright", legend = c("VE, heterogeneous risk"), col = c("blue"), lwd = 2, cex = 0.9, bty = "n")
```
