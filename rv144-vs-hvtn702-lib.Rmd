...
si.ode.rv144.hvtn702.fn <- function ( times, init, param ) {
  with(as.list(c(init, param)), {
    ...
    #rv144.SIpm.flow <- (10^rv144.log10lambda)*rv144.Spl ### BUG HERE: Spl instead of Spm!!!
...
The "mod" object is produced after each model run; it is an Epimodel output object. The following mod.manipulate(..) function takes the mod output and uses it to estimated incidence and vaccine efficacy and add those values to the mod object.
...
Ok, so there are two sets of model-specific parameters (based on RV144 and HVTN702), and then one parameter that is shared (epsilon, the true per-contact vaccine efficacy), so the best way to optimize is using conditional optimization. That is, we first hold epsilon fixed and do each model optimization separately, then hold those resulting parameters fixed and optimize epsilon, and iterate
until convergence.

WHAT do all of these functions do?

Where did the "other.parameters[[]]" object come from? Should it be "all.parameters[[]]"?
...
#Does the following run the whole simulation? 
This a single function?!?!

I am so confused by the following steps.
...
num.sims <- 15; # Fast for superfast debugging.
...
RV144 placebo incidence in the prespecified analysis (MITT) cohort was 0.1397 per 100 person years, 100*(74/52985) from NEJM 2009; 361:2209-2220 DOI: 10.1056/NEJMoa0908492 December 3, 2009 (https://www.nejm.org/doi/full/10.1056/nejmoa0908492):
...
HVTN702 placebo incidence was 3.3 per 100 person-years (95% CI, 2.8 to 3.9), from NEJM 384;12 nejm.org March 25, 2021 (https://www.nejm.org/doi/pdf/10.1056/NEJMoa2031499), page 1092:
