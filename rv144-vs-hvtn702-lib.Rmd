...
si.ode.rv144.hvtn702.fn <- function ( times, init, param ) {
  with(as.list(c(init, param)), {
    ...
    # RV144 (HIV vaccine trial that took place in Thailand)
...
### DARN it actually had TWO BUGS! Also the l at the end! 
    ...
    # HVTN 702 (HIV vaccine trial that took place in South Africa)
...
# Manipulate the model output

The "mod" Epimodel object is produced after each model run. The following mod.manipulate(..) function takes the mod output and uses it to estimate incidence and (estimated/clinical/observed) vaccine efficacy and add those values to the mod object.

Usage: \>mod.manipulate (..)

```{r mod-manipulate-function}
...
This computes the distance as calculated internally in the abc function - but not where the distances are normalized using "normalise", which we think centralizes also (so makes each scaled stat have mean 0, sd 1) - the standard deviations are saved and included in the abc output and need to be passed into here, because after keeping only the closest X% of the samples, the remaining samples will have a different STDEV. So to get the right distances it's important to use the correct stdev. These values are not centralized before the distances are computed but this should not matter.

Usage: \>calculate.abc.dist ( .. ) Example: \>calculate.abc.dist( fit.rej$stats, as.numeric( target.stats ), fit.rej$stats_normalization )

```{r calculate-abc-dist}
...
# Running the trial model

Usage: \>run.and.compute.run.stats (..)

```{r run-and-compute-stats}
run.and.compute.run.stats <- function (
      epsilon,          #per contact vaccine efficacy; true VE
...
# The model function call dcm(..) includes: "param", "init", and "control": "dcm(param, init, control)"; this is Epimodel structure.
  
# Paul added the other params to this (risk here, others below):

### Param ###  

# defining the param object
...
# Defining the init object
      ...
      ## The placebo incidence out stat vector is the -cumulative- incidence at time trial.evaluation.time.
...
Ok, so there are two sets of model-specific parameters (based on RV144 and HVTN702), and then one parameter that is shared (*epsilon*, the per-exposure vaccine efficacy), so the best way to optimize is using conditional optimization. That is, we first hold *epsilon* fixed and do each model optimization separately, then hold those resulting parameters fixed and optimize *epsilon*, and iterate until convergence.

```{r optimization-functions}
make.epsilon.abc.fn <- function ( other.parameters ) {
        function( x ) {   # this function calls the run.and.compute.run.stats() fn
