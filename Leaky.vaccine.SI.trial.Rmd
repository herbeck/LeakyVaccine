...
```{r data}
mod <- mutate_epi(mod, num.Sp.flow = Sp + SIp.flow) #all susceptible and new infections in homogeneous risk placebo pop, at each time step
mod <- mutate_epi(mod, num.Sv.flow = Sv + SIv.flow) #all susceptible and new infections in homogeneous risk vaccinated pop
mod <- mutate_epi(mod, num.Svh.Svl.flow = Svh + Svl + SIvh.flow + SIvl.flow) #all susceptible and new infections in heterogeneous risk vaccine pop
mod <- mutate_epi(mod, num.Sph.Spl.flow = Sph + Spl + SIph.flow + SIpl.flow) #all susceptible and new infections in heterogeneous risk placebo pop
...
mod <- mutate_epi(mod, rate.Vaccine = (SIv.flow/num.Sv.flow)*100)
mod <- mutate_epi(mod, rate.Placebo = (SIp.flow/num.Sp.flow)*100)
mod <- mutate_epi(mod, rate.Vaccine.het = (SIvh.flow/num.Svh.Svl.flow)*100)
mod <- mutate_epi(mod, rate.Placebo.het = (SIph.flow/num.Sph.Spl.flow)*100)
